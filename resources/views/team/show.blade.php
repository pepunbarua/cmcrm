<x-dashboard-layout>
    <div class="p-6">
            ...
            <x-card>
                <div class="text-center py-12">
                    ...
                    <div class="flex gap-3 justify-center">
                        ...
                        <form action="{{ route('team.destroy', ['team' => $team]) }}" method="POST" class="inline-block">
                            @csrf
                            @method('DELETE')
        ...
        <div class="mb-6">
            ...
            <div class="flex justify-between items-start">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ $team->user->name }}</h1>
                    <p class="text-gray-600 dark:text-white/60 capitalize">{{ str_replace('_', ' ', $team->role_type) }}</p>
                </div>
                <div class="flex gap-2">
                    @can('edit team members')
                    <x-button href="{{ route('team.edit', ['team' => $team]) }}" variant="secondary">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
            ...
            <x-stat-card 
                title="Completed" 
                :value="(string)$stats['completed_events']" 
                icon="check-circle"
                color="green"
            />
            <x-stat-card 
            ...
            <x-stat-card 
                title="In Progress" 
                :value="(string)$stats['in_progress_events']" 
                icon="play"
                color="yellow"
            />
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-1 space-y-6">
                ...
                <x-card>
                    <div class="text-center mb-6">
                        <div class="w-24 h-24 rounded-full bg-gradient-to-br from-purple-500 to-pink-500 flex items-center justify-center text-white font-bold text-3xl mx-auto mb-4">
                            {{ strtoupper(substr($team->user->name, 0, 2)) }}
                        </div>
                        
                        <!-- Availability Badge -->
                        <div class="inline-flex items-center gap-2 px-4 py-2 rounded-full mb-4
                            @if($team->availability_status == 'available') bg-green-500/20 text-green-600 dark:text-green-400
                            @elseif($team->availability_status == 'busy') bg-yellow-500/20 text-yellow-600 dark:text-yellow-400
                            @else bg-red-500/20 text-red-600 dark:text-red-400
                            @endif">
                            <div class="w-2 h-2 rounded-full 
                                @if($team->availability_status == 'available') bg-green-500
                                @elseif($team->availability_status == 'busy') bg-yellow-500
                                @else bg-red-500
                                @endif">
                            </div>
                            <span class="font-medium capitalize">{{ str_replace('_', ' ', $team->availability_status) }}</span>
                        </div>
                    ...
                    <div class="space-y-3 text-sm">
                        ...
                        <div class="flex items-center gap-3 text-gray-600 dark:text-white/60">
                            ...
                            <span class="capitalize">{{ str_replace('_', ' ', $team->role_type) }}</span>
                        </div>
                        <div class="flex items-center gap-3 text-gray-600 dark:text-white/60">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4M7.835 4.697a3.42 3.42 0 001.946-.806 3.42 3.42 0 014.438 0 3.42 3.42 0 001.946.806 3.42 3.42 0 013.138 3.138 3.42 3.42 0 00.806 1.946 3.42 3.42 0 010 4.438 3.42 3.42 0 00-.806 1.946 3.42 3.42 0 01-3.138 3.138 3.42 3.42 0 00-1.946.806 3.42 3.42 0 01-4.438 0 3.42 3.42 0 00-1.946-.806 3.42 3.42 0 01-3.138-3.138 3.42 3.42 0 00-.806-1.946 3.42 3.42 0 010-4.438 3.42 3.42 0 00.806-1.946 3.42 3.42 0 013.138-3.138z"/>
                            </svg>
                            <span class="capitalize">{{ str_replace('_', ' ', $team->skill_level) }}</span>
                        </div>
                        @if($team->hourly_rate)
                        <div class="flex items-center gap-3 text-gray-600 dark:text-white/60">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <span>৳{{ number_format($team->hourly_rate, 2) }}/hour</span>
                        </div>
                        @endif
                    </div>
                </x-card>
                ...
                <x-card>
                    ...
                    <p class="text-sm text-gray-600 dark:text-white/60 whitespace-pre-line">{{ $team->equipment_owned }}</p>
                </x-card>
                @endif
                ...
                <x-card>
                    ...
                    <a href="{{ $team->portfolio_link }}" target="_blank" class="text-sm text-purple-600 dark:text-purple-400 hover:underline flex items-center gap-2">
                        {{ $team->portfolio_link }}
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                @if($team->assignedEventsAsPhotographer->count() > 0)
                <x-card>
                    <h3 class="font-semibold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
                        <svg class="w-5 h-5 text-purple-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 9a2 2 0 012-2h.93a2 2 0 001.664-.89l.812-1.22A2 2 0 0110.07 4h3.86a2 2 0 011.664.89l.812 1.22A2 2 0 0018.07 7H19a2 2 0 012 2v9a2 2 0 01-2 2H5a2 2 0 01-2-2V9z"/>
                        </svg>
                        Events as Photographer ({{ $team->assignedEventsAsPhotographer->count() }})
                    </h3>
                    <div class="space-y-3">
                        @foreach($team->assignedEventsAsPhotographer->take(5) as $event)
                ...
                @if($team->assignedEventsAsVideographer->count() > 0)
                <x-card>
                    <h3 class="font-semibold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
                        <svg class="w-5 h-5 text-pink-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z"/>
                        </svg>
                        Events as Videographer ({{ $team->assignedEventsAsVideographer->count() }})
                    </h3>
                    <div class="space-y-3">
                        @foreach($team->assignedEventsAsVideographer->take(5) as $event)
                ...
                <!-- No Events -->
                @if($team->assignedEventsAsPhotographer->count() == 0 && $team->assignedEventsAsVideographer->count() == 0)
                <x-card>
    ...
    <script>
        async function updateAvailability(status) {
            try {
                const response = await fetch('{{ route("team.update-availability", ['teamMember' => $team]) }}', {
                    method: 'POST',
                    headers: {
        ...
        async function deleteMember() {
            ...
            try {
                const response = await fetch('{{ route("team.destroy", ['team' => $team]) }}', {
                    method: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': '{{ csrf_token() }}',
                        'Accept': 'application/json',
